...
- 无参SQL固定宽度导出测试,固定宽度20,也可以使用setWidth(int)来重置列宽
```
...
- SQL带参数测试，且将满足条件的单元格标红。如果某个列字符串重复率很高时可以将其设为共享达到数据压缩的目的。
```
try (Connection con = dataSource.getConnection()) {
...
- 对象数组 & Map数组支持。对象可以通过注解@DisplayName来设置表头列或共享，使用@NotExport来指定不导出的字段。
```
    /**
     * 测试对象
     */
